...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(`🧹 去重后剩余: ${uniqueMarkets.length} 个唯一比赛`);
    ...
    for (const market of uniqueMarkets) {
      ...
      // ⏰ 关键逻辑：如果比赛已经开始，跳过更新 (锁定赛前预测)
      // 这样数据库里留下的就是比赛开始前最后一次生成的预测，保证统计真实性
      const startTime = new Date(market.startTime).getTime();
      const now = Date.now();

      if (startTime <= now) {
         // 比赛已开始，不做更新，保留 Redis 中的旧数据供结算使用
         console.log(`🔒 比赛已开始，停止更新 (锁定赛前预测): ${market.teamA.name} vs ${market.teamB.name}`);
         continue;
      }
      ...
      try {
        ...
        if (prediction && prediction.confidence > 0.5) {
          ...
          console.log(
            `✅ [${updatedCount}/${uniqueMarkets.length}] ${market.teamA.name} vs ${market.teamB.name} | ${homeLabel} | Win%=${(prediction.teamAProbability * 100).toFixed(1)}% | Confidence=${(prediction.confidence * 100).toFixed(0)}%`
          );
        } else {
          console.log(
    ...
    console.log(`📊 总市场数 (原始/去重): ${markets.length} / ${uniqueMarkets.length}`);
