...
import { useEffect, useState, useCallback } from 'react'; // 导入 useCallback 用于 getHomeAwayBadge
...
export default function MarketDetailPage({ params }: { params: { id: string } }) {
  ...
  async function loadData() {
    try {
      ...
      const eventId = await findEspnGame(teamAName, teamBName, foundMarket.startTime);

      let currentEnhancedData = null;
      let homeFlag: boolean | null = null; // NEW: 用于 loadData 内部传递

      if (eventId) {
        const gameDataRes = await fetch(`/api/game-data?eventId=${eventId}`);
        if (gameDataRes.ok) {
          ...
          if (seasonEvent) {
            // 查找 Team A 在 ESPN 数据中的身份 (Home/Away)
      ...
      // 5. Prediction
      if (currentStats) {
        const injA = currentEnhancedData?.injuries?.find((i: any) => i.teamName.includes(teamAName.split(' ')[1]) || i.teamName === teamAName);
        const injB = currentEnhancedData?.injuries?.find((i: any) => i.teamName.includes(teamBName.split(' ')[1]) || i.teamName === teamBName);
        ...
        const pred = generatePrediction(
          teamAName, teamBName, currentStats, statsA, statsB,
          injA || null, injB || null,
          foundMarket.prices,
          restA,
          restB,
  ...
  const calculateRestDaysNum = (teamName: string, games: H2HGame[], marketStartTime?: string): number => {
    ...
    // Filter out future games (if any exist in the list that are newer than market start time)
    const gameTime = marketStartTime ? new Date(marketStartTime) : new Date();
    const pastGames = sortedGames.filter(g => new Date(g.date).getTime() < gameTime.getTime());

    if (pastGames.length === 0) return 3;

    const lastGame = new Date(pastGames[0].date);
    const diffTime = Math.abs(gameTime.getTime() - lastGame.getTime());
  ...
  // NEW: Badge generator
  const getHomeAwayBadge = (isHome: boolean | null, labelTeam: 'A' | 'B') => {
  ...
  if (error || !market) return <div className="min-h-screen bg-slate-950 flex justify-center items-center text-red-500">{error || '无数据'}</div>;
  ...
  const chartData = h2hGames
    ...
    .map((g, i) => ({
      ...
      [teamA]: g.home === teamA ? g.homeScore : g.awayScore,
      [teamB]: g.home === teamB ? g.homeScore : g.awayScore,
    }));
             ...
             <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                 ...
                 <div className="flex items-center gap-6 flex-1 justify-end">
                     <div className="text-right hidden md:block">
                         ...
                         <div className="text-blue-400 font-mono text-sm mt-1 font-medium">{getRestDaysText(teamA)} 休息</div>
                     </div>
                     <img src={getTeamLogoUrl(teamA)} className="w-20 h-20 object-contain drop-shadow-2xl" alt=""/>
                 </div>

                 {/* Score/Odds Center */}
                 <div className="flex flex-col items-center shrink-0 px-8">
                     <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Polymarket Odds</div>
                     <div className="flex items-center gap-0 bg-slate-950 rounded-xl border border-slate-700 overflow-hidden">
                         <div className="px-6 py-3 text-center border-r border-slate-800 hover:bg-slate-900 transition-colors cursor-default">
                             <div className="text-2xl font-bold text-blue-500">{(market.prices.yes * 100).toFixed(0)}%</div>
                         ...
                         <div className="px-6 py-3 text-center hover:bg-slate-900 transition-colors cursor-default">
                             <div className="text-2xl font-bold text-red-500">{(market.prices.no * 100).toFixed(0)}%</div>
                 ...
                 <div className="flex items-center gap-6 flex-1">
                     <img src={getTeamLogoUrl(teamB)} className="w-20 h-20 object-contain drop-shadow-2xl" alt=""/>
                     <div className="hidden md:block">
                         ...
                         <div className="text-blue-400 font-mono text-sm mt-1 font-medium">{getRestDaysText(teamB)} 休息</div>
                     </div>
                 </div>
             </div>
        </div>
            ...
            <div className="lg:col-span-7 bg-slate-900 rounded-2xl border border-slate-800 shadow-xl p-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 relative z-10">
                    {prediction?.factors.slice(0, 4).map((f, i) => (
                        <div key={i} className="bg-slate-950/80 p-4 rounded-xl border border-slate-800 hover:border-slate-700 transition-colors">
                            <div className="flex justify-between mb-2">
                                <span className="text-xs font-bold text-slate-300 uppercase tracking-wide">{f.name}</span>
                                <span className={`text-xs font-bold ${f.score > 0 ? 'text-blue-400' : 'text-red-400'}`}>
                                    {f.score > 0 ? teamA : teamB}
                                </span>
                            </div>
                            <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden mb-2">
                                <div 
                                    className={`h-full rounded-full ${f.score > 0 ? 'bg-blue-500' : 'bg-red-500'}`} 
                   ...
                   <div className="p-6 flex-1 flex flex-col justify-center space-y-6">
                       ...
                       <div className="flex justify-between items-center pb-4 border-b border-slate-800/50">
                           ...
                           <div className="font-mono text-sm font-bold">
                               <span className="text-blue-400 mr-2">{(market.prices.yes * 100).toFixed(0)}%</span>
                               <span className="text-slate-700">/</span>
                               <span className="text-red-400 ml-2">{(market.prices.no * 100).toFixed(0)}%</span>
                           </div>
                       </div>
                       ...
                       <div className="flex justify-between items-center">
                           ...
                           <span className={`text-xl font-mono font-black ${diff > 0 ? 'text-green-400' : 'text-red-400'}`}>
                               {diff > 0 ? '+' : ''}{diff.toFixed(1)}%
                           </span>
                       </div>

                       {/* Conclusion Box */}
                       <div className={`mt-4 p-4 rounded-xl border text-center transition-all duration-500 ${
                           Math.abs(diff) > 5 
                           ...
                           {diff > 5 ? (
                               <div>
                                   ...
                                   <div className="text-xs opacity-70 mt-1 font-mono uppercase">Strong Buy Signal</div>
                               </div>
                           ) : diff < -5 ? (
                               <div>
                                   <div className="text-2xl mb-1">💰</div>
                               ...
                               <div>
                                   ...
                                   <div className="text-xs opacity-70 mt-1 font-mono uppercase">No Clear Edge</div>
                               </div>
                           )}
                       </div>
                   </div>
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-slate-950 rounded-xl p-5 border border-slate-800 hover:border-slate-700 transition-colors">
                        <div className="text-xs font-bold text-slate-500 uppercase mb-5 text-center tracking-widest">进攻效率</div>
                        <StatRow label="3P%" valA={advancedStatsA.threePointPct} valB={advancedStatsB.threePointPct} suffix="%" />
                        <StatRow label="eFG%" valA={advancedStatsA.effectiveFGPct} valB={advancedStatsB.effectiveFGPct} suffix="%" />
                        <StatRow label="Assists" valA={advancedStatsA.avgAssists} valB={advancedStatsB.avgAssists} />
                    </div>
                    {/* Defensive */}
                    <div className="bg-slate-950 rounded-xl p-5 border border-slate-800 hover:border-slate-700 transition-colors">
                        <div className="text-xs font-bold text-slate-500 uppercase mb-5 text-center tracking-widest">防守效率</div>
                        <StatRow label="Steals" valA={advancedStatsA.avgSteals} valB={advancedStatsB.avgSteals} />
                        <StatRow label="Blocks" valA={advancedStatsA.avgBlocks} valB={advancedStatsB.avgBlocks} />
                        <StatRow label="Def Reb" valA={advancedStatsA.avgDefensiveRebounds} valB={advancedStatsB.avgDefensiveRebounds} />
                    </div>
                    {/* Key Metrics */}
                    <div className="bg-slate-950 rounded-xl p-5 border border-slate-800 hover:border-slate-700 transition-colors">
                        <div className="text-xs font-bold text-slate-500 uppercase mb-5 text-center tracking-widest">关键指标</div>
                        <StatRow label="TO Ratio" valA={advancedStatsA.assistTurnoverRatio} valB={advancedStatsB.assistTurnoverRatio} inverse />
                    ...
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                            {stats?.recentForm.teamA.split('').map((r, i) => (
                                <span key={i} className={`w-6 h-6 flex items-center justify-center text-[10px] font-black rounded ${
                                    r === 'W' 
                ...
                <div className="h-48 w-full mt-8">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData}>
                            ...
                            <Line type="monotone" dataKey={teamB} stroke="#dc2626" strokeWidth={2} dot={false} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </div>
                        ...
                        {enhancedData.injuries.map((teamInj) => (
                            <div key={teamInj.teamId}>
                                <div className="flex items-center gap-2 mb-3 sticky top-0 bg-slate-900 py-2 z-10 border-b border-slate-800">
                                    <img src={getTeamLogoUrl(teamInj.teamName)} className="w-5 h-5 object-contain" alt=""/>
                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{teamInj.teamName}</span>
                                </div>
                                <div className="space-y-3">
                                    {teamInj.injuries.length === 0 ? (
                                        ...
                                        teamInj.injuries.map((p) => {
                                            ...
                                            return (
                                            <div key={p.athleteId} className="flex items-center gap-3 p-3 bg-slate-950 rounded-xl border border-slate-800 hover:border-slate-700 transition-colors group">
                                                <div className="w-10 h-10 bg-slate-900 rounded-full overflow-hidden border border-slate-800 shrink-0 group-hover:border-slate-600 transition-colors">
                                                    {p.headshot ? <img src={p.headshot} className="w-full h-full object-cover" alt={p.athleteName}/> : <div className="w-full h-full flex items-center justify-center text-xs text-slate-600">?</div>}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="font-bold text-sm text-slate-200 truncate">{p.athleteName}</div>
                        ...
                        {h2hGames.map((game, index) => (
                            <tr key={index} className="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <div className={`flex items-center gap-2 ${game.home === game.winner ? 'font-bold text-white' : ''}`}>
                                        {game.home === game.winner && <span className="text-yellow-500">👑</span>}
                                        {game.home}
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4">
                                    <div className={`flex items-center justify-end gap-2 ${game.away === game.winner ? 'font-bold text-white' : ''}`}>
                                        {game.away}
                                        {game.away === game.winner && <span className="text-yellow-500">👑</span>}
                                    </div>
                                </td>
                            </tr>
                        ))}
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-lg">
            ...
            <div className="space-y-3 text-sm text-slate-300 leading-relaxed">
                {prediction?.reasoning.map((text, i) => (
                    <div key={i} className="flex gap-3 p-2 hover:bg-slate-950/50 rounded-lg transition-colors">
    ...
    return (
        <div className="bg-slate-950 rounded-xl p-5 border border-slate-800 hover:border-slate-700 transition-all group">
            <div className="flex justify-between items-center mb-3">
                <span className="font-bold text-slate-200 group-hover:text-white transition-colors">{team}</span>
            ...
            <div className="mt-3 h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${days.includes('1') ? 'bg-red-500' : 'bg-green-500'}`} style={{width: days.includes('1') ? '10%' : '100%'}}></div>
            </div>
        </div>
    )
}

function StatRow({ label, valA, valB, suffix = '', inverse = false }: { label: string, valA?: number, valB?: number, suffix?: string, inverse?: boolean }) {
    ...
    return (
        <div className="flex justify-between items-center mb-3 text-sm last:mb-0">
            <span className={`font-mono font-bold ${isABetter ? 'text-green-400' : 'text-slate-500'}`}>
                {valA.toFixed(1)}{suffix}
            </span>
            <span className="text-[10px] font-bold text-slate-600 uppercase bg-slate-900 px-2 py-0.5 rounded">{label}</span>
            <span className={`font-mono font-bold ${!isABetter ? 'text-green-400' : 'text-slate-500'}`}>
